...
def test_load_image_wrong_type(classifier, url):
...
def test_load_image_invalid_url(classifier, url):
    with pytest.raises(Exception) as e_info:
        image = classifier.load_image(url)
...
@pytest.mark.parametrize('url', image_urls)
def test_load_image_file_object(classifier, url):
    fobj = urllib.request.urlopen(url)
    image = classifier.load_image(fobj)
    assert image.shape == (224, 224, 3)
    assert np.all(image <= 1)  # normalized
...
def test_identify_wrong_type(classifier, image_url):
    with pytest.raises(Exception) as e_info:
...
def test_identify_invalid_image_url(classifier, image_url):
    with pytest.raises(Exception) as e_info:
        top_names, top_scores = classifier.identify(image_url)
...
def test_identify_file_object(classifier, image_url, exp_top_3_names):
    fobj = urllib.request.urlopen(image_url)
    top_names, top_scores = classifier.identify(fobj)
    npt.assert_equal(top_names[:3], exp_top_3_names)
    npt.assert_array_less(top_scores[1:]/top_scores[0], 0.5)  # top probability more than twice others
    npt.assert_almost_equal(sum(top_scores), 1, decimal=5)  # total probability 1